...
When reading from csv files use `r RDoc("read::read_csv")` instead of the base R function `r RDoc("base::read.csv")`. 
It is slightly faster, and returns a `tibble` instead of a data frame.
See  `r r4ds_ch("Data Import", "data-import")` for more dicussion.
...
Load the data using the `r pkg("readr")` function `r RDoc("readr::read_csv")`
```{r message=FALSE}
...
Load the `minwage` dataset from its URL using `r RDoc("readr::read_csv")`:
```{r}
minwage <- read_csv(qss_data_url("causality", "minwage.csv"))
glimpse(minwage)
summary(minwage)
```
