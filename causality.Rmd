...
A way to do this using the `r rdoc("tidyr", "spread")` and `r rdoc("tidyr", "gather")` functions from `r pkg("tidyr")` package.
See the `r R4DS` chapgter ["Tidy Data"](http://r4ds.had.co.nz/tidy-data.html).
...
**WARNING** The function `r rdoc("dplyr", "ungroup")` removes the groupings 
in `r rdoc("dplyr", "group_by")`. The function `spread` will not allow a grouping
...
Use  `r rdoc("stringr", "str_to_title")` to capitalize `sex` and `race`,and `r rdoc("stringr", "str_c")` to concatenate these vectors.
```{r}
resume <-
  resume %>%
