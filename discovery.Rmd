...
tokens
```
The `unnest_tokens` function uses the `r pkg_link("tokenizers")` package to tokenize the text.
...
The `r pkg_link("igraph")`, `r pkg_link("sna")`, and `r pkg_link("network")` packages are the best in class. 
...
- `r pkg_link("ggnetwork")`
- `r pkg_link("ggraph")`
- `r pkg_link("geomnet")`
- `r pkg_link("GGally")` functions [ggnet](https://ggobi.github.io/ggally/rd.html#ggnet), `ggnet2`, and `ggnetworkmap`.
...
- `r pkg_link("tmap")` is not built on ggplot2 but uses a ggplot2-like API for network data.
- `r pkg_link("leaflet")` is an R interface to a popular javascript mapping library.
...
- See the `r pkg("viridis")`, `r pkg("ggthemes")`, and `r pkg("pals")` packages for color palettes.
