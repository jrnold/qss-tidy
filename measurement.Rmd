...
to the same type, e.g. `r rdoc("purrr", "map")` and most `r pkg("purrr")` functions,
and `if_else`.
...
dens_plot
```
which can be combined with a `r rdoc("ggplot2", "geom_rug")` to show individual values:
```{r}
dens_plot + geom_rug()
```
...
See `r R4DS` chapters [Vectors](http://r4ds.had.co.nz/vectors.html), [Iteration](http://r4ds.had.co.nz/iteration.html) and the `r pkg("purrr")` package for more powerful methods of computing on lists.
